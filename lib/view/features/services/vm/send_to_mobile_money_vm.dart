...
class SendToMobileMoneyNotifier extends AutoDisposeAsyncNotifier<SendMoneyResponse> {
  Future<void> sendToMobileMoneyMethod(SendToMobileMoneyReq req) async {
    state = const AsyncLoading();
    state = await AsyncValue.guard(
      () => ref.read(transferRepository).sendMoneyToMobileMoneyEndpoint(req),
    );
    if (!state.hasError) {
...
final sendToMobileMoneyProvider =
    AsyncNotifierProvider.autoDispose<SendToMobileMoneyNotifier, SendMoneyResponse>(
        SendToMobileMoneyNotifier.new);
